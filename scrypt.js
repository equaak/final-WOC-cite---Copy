...
$(document).scroll(function(){
        if(window.scrollY > height){
            ...
            $('.new_header').css('background-color', '#4e279e')
...
function EditAll(){
    ...
    for(let i = 0; i < 10; i++){
        const gallery_phone_list_fourth = document.querySelector('.gallery_phone_list4')
        let random = generateRandomInteger(1, phone_photo_mas.length - 1)
        gallery_phone_list_fourth.insertAdjacentHTML('beforeend',

        `

            <div class="phone_gallery_list_img4${i} phone_gallery_list_img"></div>

        `

        )

        let phone_gallery_list_img1 = document.querySelector(`.phone_gallery_list_img4${i}`)
        $(phone_gallery_list_img1).css('background-image', `url(img/insta_img${phone_photo_mas[random]}.jpg)`)
        $(phone_gallery_list_img1).css('background-size', 'cover')
        phone_photo_mas = removeItem(phone_photo_mas, random)
    }
    ...
    for(let i = 0; i < 10; i++){
        const gallery_phone_list_fifth = document.querySelector('.news-ticker-h')
        let random = generateRandomInteger(1, phone_photo_mas.length - 1)
        gallery_phone_list_fifth.insertAdjacentHTML('beforeend', `
        <div class="phone_gallery_list_img5${i} phone_gallery_list_img"></div>
        `)
        let phone_gallery_list_img1 = document.querySelector(`.phone_gallery_list_img5${i}`)
        $(phone_gallery_list_img1).css('background-image', `url(img/insta_img${phone_photo_mas[random]}.jpg)`)
        $(phone_gallery_list_img1).css('background-size', 'cover')
        phone_photo_mas = removeItem(phone_photo_mas, random)
    }
    ...
    for(let i = 0; i < 15; i++){
        const gallery_rect2 = document.querySelector('.gallery_rect4')
        let random = generateRandomInteger(1, photo_mas.length)
        gallery_rect2.insertAdjacentHTML('beforeend',

        `

            <div class="gallery_list_img8${i} gallery_list_img"></div>

        `

        )

        let gallery_list_img = document.querySelector(`.gallery_list_img8${i}`)
        $(gallery_list_img).css('background-image', `url(img/insta_img${photo_mas[random]}.jpg)`)
        $(gallery_list_img).css('background-size', 'cover')
        photo_mas = removeItem(photo_mas, random)
    }
...
let gallery4 = document.querySelector('.gallery_all4')
, glist4 = document.querySelector('.gallery_rect4')
, gclone4 = glist4.cloneNode(true)
gallery4.append(gclone4)
...
let gallery_animation_fourth = document.querySelector('.gallery_animation_fourth'),
gallery_phone_list_fourth = document.querySelector('.gallery_phone_list4'),
gallery_animation_clone_fourth = gallery_phone_list_fourth.cloneNode(true)
gallery_animation_fourth.append(gallery_animation_clone_fourth)

let ticker_wrapper_h = document.querySelector('.ticker-wrapper-h'),
news_ticker_h = document.querySelector('.news-ticker-h'),
news_ticker_h_clone = news_ticker_h.cloneNode(true)
...
team_left.addEventListener('onclick', () => {
    const team_block = document.querySelector('.team_members')
    team_block.css('translateX', '25vw')
})
